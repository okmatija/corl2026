...
2. Decide what each item changes. Read the reason carefully - a 👎 with a reason means "apply the reason", not "delete the item".
   ...
   Votes (`vote`): `add` (🗓️ "Add to plan" on an idea) → fit it into the plan; `delete` (🗑️ on an idea card, sent without a pop-up) → delete that idea
   card from `ideas` (and take it out of the plan if it is in it) - UNLESS someone has replied on that issue asking to undo/keep it,
   in which case keep it and resolve as not planned. `remove` (tapping 🗓️ "Planned") → take it out
...
4. Prepend an entry to `window.RUNS` in `data/runs.js`: start/end ISO UTC times (`date -u +%Y-%m-%dT%H:%M:%SZ` at start and just
   ...
   EVERY change pushed to the site gets one such card - plan updates and website changes alike (interactive sessions
   too; `issues: []` if no comment was involved). Past cards are READ-ONLY: never edit, reword or delete an existing
   entry in `window.RUNS` - only prepend new ones.
...
- 📜 Details pages are a standard idiom: add an entry to `TRIP.details` keyed like a plan target (`stop:<place>`,
  `travel:<place>`, `travel:home`), or `stay:<place>` / `idea:<idea id>` with `{ title, intro, sections: [{ title, items: [{ name, text, link }] }] }` (or `{ href }`
